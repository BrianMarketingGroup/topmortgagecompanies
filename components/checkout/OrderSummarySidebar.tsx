...
import { calculateQuote, computeExcludedFeatured, formatCurrency } from "@/lib/pricing";
...
export default function OrderSummarySidebar({ config }: { config: SiteConfig }) {
  ...
  const productOptions = config.specialty?.options ?? [];
  const selectedProductLabels = productOptions
    .filter((o) => specialtyIds.includes(o.id))
    .map((o) => o.label);

  const featured = selectedMarkets.some((m) => m.featuredAreaIds.length > 0);
  const excludedFeatured = computeExcludedFeatured(selectedMarkets, productOptions, specialtyIds);

  const quote = calculateQuote({
    loanProducts: selectedProductLabels,
        ...
        {config.specialty && (
          <div className="flex justify-between gap-4">
            ...
            <dd className="text-right text-dark font-medium">
              {selectedProductLabels.length > 0 ? selectedProductLabels.join(", ") : "—"}
            </dd>
          </div>
        )}
        <div className="flex justify-between gap-4">
